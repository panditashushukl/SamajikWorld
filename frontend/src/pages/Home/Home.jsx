...
import CreatePlaylistDialog from "../../components/Dialogs/CreatePlaylistDialog/CreatePlaylist";
...
function Home() {
    ...
    const [activeTab, setActiveTab] = useState("all"); // all, video, tweet
    ...
    useEffect(() => {
        (async () => {
            try {
                setLoading(true);
                await dispatch(fetchHomeVideos(activeTab));
                setLoading(false);
            } catch (error) {
                console.error(
            ...
            <div className="h-screen overflow-y-scroll scrollbar-hide bg-[#121212] text-white">
                <div className="flex min-h-[calc(100vh-66px)] sm:min-h-[calc(100vh-82px)]">
                    <section className="w-full pb-[70px] sm:ml-[70px] sm:pb-0 lg:ml-0">
                        {/* Tabs */}
                        <div className="sticky top-0 z-10 bg-[#121212] pt-4 px-4 pb-2 border-b border-neutral-800 mb-4">
                            <div className="flex gap-2 overflow-x-auto scrollbar-hide">
                                {["all", "video", "tweet"].map((tab) => (
                                    <button
                                        key={tab}
                        ...
                        <div className="grid grid-cols-[repeat(auto-fit,_minmax(350px,_1fr))] gap-4 p-4">
                            {loading
                                ...
                                : homeVideos?.map((item) => {
                                    if (item.type === 'tweet') {
                                        return <TweetCard key={item._id} tweet={item} />
                                    }
